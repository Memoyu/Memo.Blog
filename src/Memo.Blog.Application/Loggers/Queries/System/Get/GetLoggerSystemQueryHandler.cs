...
public class GetLoggerSystemQueryHandler(
    IMapper mapper,
    IBaseMongoRepository<LoggerSystemCollection> systemLogMongoResp) : IRequestHandler<GetLoggerSystemQuery, Result>
{
    public async Task<Result> Handle(GetLoggerSystemQuery request, CancellationToken cancellationToken)
    {
        ...
        var logs = await systemLogMongoResp.FindListAsync(f, cancellationToken: cancellationToken)?? throw new ApplicationException("系统日志不存在");
