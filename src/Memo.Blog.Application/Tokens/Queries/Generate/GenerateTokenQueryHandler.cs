    ...
    ) : IRequestHandler<GenerateTokenQuery, Result>
{
    ...
    {
        var user = await userRepo.Where(u => u.Username.Equals(request.Username)).FirstAsync();
        if (user is null)
            throw new ApplicationException("用户名或密码错误");

        var identity = await userIdentityRepo.Where(u => u.UserId == user.UserId).FirstAsync();
