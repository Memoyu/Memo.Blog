...
public class BaseDefaultRepositoryCopy<TEntity>(UnitOfWorkManager unitOfWorkManager, ICurrentUserProvider currentUserProvider, IPublisher publisher) : DefaultRepository<TEntity, long>(unitOfWorkManager?.Orm, unitOfWorkManager), IBaseDefaultRepository<TEntity> where TEntity : BaseAuditEntity
{
    /// <summary>
    ///  当前登录人信息
    /// </summary>
    protected readonly CurrentUser CurrentUser = currentUserProvider.GetCurrentUser();
